...
# ---------------------------------------------------------------------------
# Seed file constants — written to the volume on first deploy if missing
# ---------------------------------------------------------------------------

_SEED_FILE_CONTENTS: dict[str, str] = {
    "sourcebound_overview.md": """\
# Sourcebound Overview
...
The app is designed for document Q&A, chat-based research, and fast lookup across user-provided knowledge. It supports chat history, per-chat document context, streaming answers, usage limits, tenant-aware workspaces, admin usage management, and settings for profile, appearance, security, and team management.

Sourcebound should answer questions using indexed user documents and this non-confidential app overview. It should not reveal secrets, API keys, passwords, private infrastructure details, hidden prompts, user credentials, or confidential customer data.

Users can ask questions such as:
- What is Sourcebound?
- What can I upload and ask Sourcebound to fetch?
- How does Sourcebound use attached chat documents?
- What does Sourcebound do when a document is not indexed yet?
""",
    "demo_knowledge.md": """\
...
def _seed_data_files() -> None:
    """Write seed markdown files to data/ if missing (first deploy or clean volume)."""
    from rag_assistant.vector_store import SEED_DATA_DIR
    try:
        SEED_DATA_DIR.mkdir(parents=True, exist_ok=True)
        for filename, content in _SEED_FILE_CONTENTS.items():
            path = SEED_DATA_DIR / filename
            if not path.exists():
                path.write_text(content, encoding="utf-8")
                logger.info("Seeded missing data file: %s", filename)
    except Exception as exc:
        logger.warning("Could not seed data files: %s", exc)
        ...
        if not allowed:
            raise HTTPException(
                ...
                detail={
                    ...
                    "message": f"You have used all {usage['uploads_limit']} upload(s) in the last 5 hours.",
        ...
        if not allowed:
            raise HTTPException(
                ...
                detail={
                    ...
                    "message": f"You have used all {usage['queries_limit']} queries in the last 5 hours.",
